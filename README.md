...
| [JIT-06](#️-jit-06-static-abstract-member-calls-and-what-t-really-is) | static abstract member calls | Keep reference-type T off the shared path (dictionary + indirect call) | ✅ | [Verified](benchmarks/results/JIT-06-StaticAbstractCall.md) |
...
**Return `Current` as `ref readonly` when the elements are structs:** when the loop body hands the element to a method (`Consume(in entry)`, 64-byte elements), an `Entry Current` copies 64 bytes to the stack on every iteration before passing them on (one `vmovdqu32 zmm` pair, 121 B on x86-64-v4; `vmovdqu` ×4, 124 B on x86-64-v3). A `ref readonly Entry Current` passes the array slot address as is (`lea rcx,[rsi+rcx+10]`, 81 B) and compiles to the same instruction sequence as `foreach (ref readonly var e in span)` and `for` + `ref readonly` local. **The time difference is core-dependent**: **0.80x** on x86-64-v3 (1.27 vs 1.62 ns per element), but on x86-64-v4 the single AVX-512 copy hides under the `Consume` call — the by-value form runs at 1.21 ns and the three ref forms land at 1.19-1.32 ns (the 10% placement spread of one and the same instruction sequence), so there is no time difference. The reason to adopt it is not time but that the per-iteration copy disappears from the generated code. A body that only reads one field copies nothing either way (R-04), so this pays off when elements are passed by reference or are large and multi-field. Callers write `foreach (ref readonly var x in ...)`, and a plain `foreach (var x in ...)` still compiles (the copy becomes the caller's choice). → [LAB-RefEnumerator](benchmarks/results/LAB-RefEnumerator.md)
...
**Effect (net10 / x86-64-v4, per call):**

| Shape | Time | Code size | What runs |
|---|---:|---:|---|
| Direct call `OpA.Compute(i)` (baseline) | 0.386 ns | 19 B | Inlined |
| Value-type T (`Saim<AddOp>` through NoInlining) | 1.182 ns (3.07x) | 43 B | Exact code; the difference is only the NoInlining call |
| **Reference-type T, inlined into the caller** (`AggressiveInlining` + exact type arguments) | **0.385 ns (1.00x)** | **19 B** | Identical to the baseline |
| **Reference-type T, the shared (`__Canon`) body executes** | **1.829 ns (4.75x)** | 354 B | Dictionary slot load + indirect `call rax`; `GenericsHelpers.Method` on the first call only |
| Same, two instantiations alternating | 2.254 ns (5.85x) | 382 B | 1.23x for the alternating indirect-call target (no difference on x86-64-v3) |
| Instance interface call (monomorphic) | 1.165 ns (3.02x) | 90 B | Devirtualized + inlined by PGO's GDV (`cmp [rcx],MT_OpA`) |
| Same (polymorphic, 2 types) | 1.726 ns (4.48x) | 109 B | One guard, the other type falls back to the interface call |
...
- A reference-type `T` call left in shared code becomes an **indirect call through the generic dictionary**, and PGO's GDV cannot rescue it because there is no receiver object to guard on. On .NET 10 it is 1.6x slower than a monomorphic interface call (which GDV inlines; 3.3x on x86-64-v3) and slower than a polymorphic one too (1.06x, CIs disjoint). Coanet reported the opposite order on .NET 7 ("slightly slower than monomorphic VSD, faster than polymorphic VSD") — the difference is dynamic PGO

**AOT:** ✅ The order does not change under NativeAOT (against the AOT run's own direct call, 0.386 ns): reference-type T in shared code is **5.27x (2.04 ns)**, the interface call 2.67x monomorphic (1.03 ns) and 2.89x polymorphic (1.12 ns). Without dynamic PGO the interface dispatch costs about the same whether monomorphic or polymorphic, and the shared SAIM is 2.0x behind the monomorphic and 1.8x behind the polymorphic call — the .NET 7 order (shared SAIM ≈ monomorphic VSD) does not come back. Value-type T (1.01 ns) and the inlined reference-type T (0.386 ns) stay at direct-call speed under AOT too.
...
**Measured (net10 / x86-64-v4):** the table above. x86-64-v3 (B550H) gives the same order with a larger shared-code gap (8.44x). → [Results](benchmarks/results/JIT-06-StaticAbstractCall.md)
...
Do not count on future JIT improvements: wherever zero allocation is required, write static + TState. → [Results](benchmarks/results/LAB-DelegateEscape.md)
...
**If you stay on the BCL `Dictionary` — key it by `RuntimeTypeHandle`:** the table above is about a hand-written table; a library that stays on `Dictionary<,>` cannot choose the hash source (the only way is a class `IEqualityComparer<Type>`, which brings back one interface dispatch). Changing the **key type itself** from `Type` to `RuntimeTypeHandle` keeps the same identity hash while putting the lookup on `Dictionary`'s value-type-key fast path.

| Path (32 hits / 8 misses, per lookup) | Hit | Miss | Code size |
|---|---:|---:|---:|
| `Dictionary<Type, T>` (baseline) | 2.905 ns | 2.292 ns | 1,037 B |
| **`Dictionary<RuntimeTypeHandle, T>`** | **2.101 ns (0.72x)** | **1.861 ns (0.81x)** | 797 B |
| `Dictionary<Type, T>` + class comparer over `TypeHandle.Value` | 2.098 ns (0.72x) | 1.867 ns (0.81x) | 593 B |
| `ConcurrentDictionary<Type, T>` (default comparer) | 2.221 ns (median 2.071) | 1.465 ns | 787 B |
| **`ConcurrentDictionary<RuntimeTypeHandle, T>`** | **1.884 ns (0.85x of the row above, median 1.715)** | 1.569 ns (1.07x) | 641 B |
| Hand-written table + `TypeHandle.Value` (same as above) | **0.760 ns (0.26x)** | **0.679 ns (0.30x)** | 195 B |

**Why it works (disassembly):** `Type.GetHashCode()`, `RuntimeHelpers.GetHashCode(type)` and `RuntimeTypeHandle.GetHashCode()` all return **the same identity hash**, so the difference is the path, not hash quality. `Dictionary<Type, T>` carries **three guarded-devirtualization type checks per lookup** (is the comparer `ObjectEqualityComparer<Type>`, is the `GetHashCode` receiver a `RuntimeType`, is the `Equals` receiver a `RuntimeType`) and **spills the key to the stack** for the constrained `GetHashCode` call. With a `RuntimeTypeHandle` key the comparer check and the spills disappear and the entry comparison becomes a single `cmp`. The identity-hash acquisition (`TryGetHashCode` call + header read + assigned check) remains, which is why it does not reach the hand-written table that replaces it with **one field load** (`mov rax,[rcx+18]`), 2.8x further ahead.

**On `ConcurrentDictionary`:** the same key-type change gives **0.85x on hit (0.83x by median) and a slightly slower 1.07x on miss** (x86-64-v3: 0.80x on hit, equal on miss) — the gain is in the per-node `Equals`, which does not run on a miss. `<Type, T>` hashes with `key.GetHashCode()` directly thanks to the `_comparerIsDefaultForClasses` flag (one GDV guard), but `Equals` still goes through the comparer with two GDV guards (comparer type, node key `RuntimeType`). `<RuntimeTypeHandle, T>` takes the value-type-key path and the node compare becomes a single `cmp` (580 B → 434 B). Note that **`ConcurrentDictionary<Type, T>` beats `Dictionary<Type, T>`** (hit 2.22 vs 2.91 ns, miss 1.47 vs 2.29 ns) — there is no interface call on its hash side. x86-64-v3 gave the same sign (3.16 vs 4.6-5.5 ns), so it is confirmed on both machines. The `ConcurrentDictionary` hit rows are bimodal between launches (2.03-2.89 ns), hence the medians alongside.

**The flip when the lookup feeds a dispatch is Zen 3-specific (`TypeKeyDispatchBenchmark`):** the table above measures the lookup alone. A real registry **invokes** what it looked up (a factory delegate). Looking up 5 types in sequence from a `ConcurrentDictionary<K, Func<object>>` and invoking the result, on HX 370 (Zen 5) the `RuntimeTypeHandle` key is 0.82x when one type repeats (2.80 vs 3.39 ns) and **still 0.82x when the 5 types alternate (4.02 vs 4.89 ns)** — the lookup gain carries through (a sweep over 1 / 2 / 3 / 5 distinct targets gives 0.77-0.88x, no flip at any count). An `IntPtr` key over `TypeHandle.Value` is 0.64x (3.11 ns, but the table no longer references the `Type`), and a `Type` key with a `TypeHandle.Value` comparer is 0.88x. On B550H (Zen 3), with the same instruction sequences, only the `RuntimeTypeHandle` key sank to **2.24x with the 5 types alternating (15.64 vs 6.98 ns)** (0.62x with a single type), and the real `ServiceRegistry.Activate` in BunnyTail.DependencyInjection reproduced it (9.79 → 18.39 ns; the same code on HX 370 gives 6.29 → 5.19 ns, 0.83x). Hash acquisition alone and the lookup alone are fast on every path and the lookup code is the same on both machines (the handle variant calls the `TryGetHashCode` FCall directly and has the shortest instruction stream). B550H isolated the cause (Run 4b): the flip appears **only with 5 distinct targets** (with 1 / 2 / 3 the handle key is 0.67-0.82x), and at 5 both the handle and the `IntPtr` variant lack a delegate-target guard yet `IntPtr` stays fast (0.76x). The trigger is **the `TryGetHashCode` FCall (a direct call into native code) sitting in the same iteration as the 5-target indirect delegate call**: adding one `RuntimeHelpers.GetHashCode(type)` to the `IntPtr`-keyed loop sinks it from 0.57x to 1.52x (6.19 → 16.56 ns), and the `Type`-keyed loop goes to 1.30x. Whether PGO emits a delegate guard is irrelevant (it varies per process and does not track the slowdown); `DOTNET_TieredPGO=0` removes the flip but makes every variant about 2x slower (the lookup is no longer inlined), so it isolates nothing. Zen 3's branch / return prediction fails on this combination (counters not collected). The `Type` key is immune because its hash is taken inside the virtually called `RuntimeType.GetHashCode`, so the FCall never appears in the loop body itself. Confirmed as Zen 3-specific; it does not generalize across cores. As corroboration, under NativeAOT — whose identity hash is managed code with no FCall — the real `ServiceRegistry.Activate` with the handle key does not flip on Zen 3 either: 0.82x (18.64 → 15.22 ns) against 0.84x on HX 370. → [Run 2 / 3b / 4b](benchmarks/results/TYP-07-TypeKey.md)

**Which to use:** to speed up a `Dictionary` / `ConcurrentDictionary` without replacing it, key by `RuntimeTypeHandle` (callers pass `type.TypeHandle`) or keep the `Type` key with a class comparer over `TypeHandle.Value` — under the JIT on Zen 5 the two tie on hit and miss (0.72x / 0.81x). But **under NativeAOT the comparer does nothing** (lookup 0.94x / 1.03x, and a 1.11x regression in the dispatch shape — AOT does not devirtualize the interface call into the comparer), while the `RuntimeTypeHandle` key gains more (`Dictionary` hit 0.50x / miss 0.62x, dispatch shape 0.79x). On the other hand, in a registry that **dispatches on** the looked-up value, only the `RuntimeTypeHandle` key sank on Zen 3 under the JIT. The one key that wins on JIT, AOT and both core generations is `IntPtr` (`TypeHandle.Value`) at 0.64-0.78x — usable only without collectible ALCs. Failing that, take the `RuntimeTypeHandle` key when you ship AOT and the comparer when you ship JIT only and cannot choose the core. For more than that, the hand-written table. **The two decisions are independent.** → [Results](benchmarks/results/TYP-07-TypeKey.md)
...
| Key type for a Type-keyed dictionary (TYP-07 supplement) | `RuntimeTypeHandle` key 0.72x on hit; a class comparer over `TypeHandle.Value` ties it | **The premise holds and the gap widens.** `Dictionary<Type>` runs 2x slower than under the JIT because the interface calls into the comparer are not devirtualized, while the `RuntimeTypeHandle` key stays on the value-type-key path at 0.50x hit / 0.62x miss. The class comparer does nothing (0.94x / 1.03x, 1.11x in the dispatch shape) | **Key by `RuntimeTypeHandle` when shipping AOT (or by `IntPtr` without collectible ALCs).** Do not take the comparer route |
...
└── benchmarks/
    ...
    ├── PerformancePatterns.AotHarness/  JIT vs NativeAOT comparison harness (links the Lab classes, no diagnoser)
