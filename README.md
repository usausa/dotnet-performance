...
| `ConcurrentDictionary<Type, T>` (default comparer, second run) | 3.156 ns | 2.050 ns | 787 B |
| **`ConcurrentDictionary<RuntimeTypeHandle, T>`** | **2.531 ns (0.80x of the row above)** | 2.565 ns ⏳❗ (StdDev 0.67, min 1.960 ≈ equal) | 641 B |
...
**On `ConcurrentDictionary` (⏳ second run):** the same key-type change gives **0.80x on hit and nothing on miss** — the gain is in the per-node `Equals`, which does not run on a miss. `<Type, T>` hashes with `key.GetHashCode()` directly thanks to the `_comparerIsDefaultForClasses` flag (one GDV guard), but `Equals` still goes through the comparer with two GDV guards (comparer type, node key `RuntimeType`). `<RuntimeTypeHandle, T>` takes the value-type-key path and the node compare becomes a single `cmp` (580 B → 434 B). **⏳❗ Unexpected:** on this machine `ConcurrentDictionary<Type, T>` hit (3.16 ns) was faster than `Dictionary<Type, T>` (4.6-5.5 ns). The missing interface call on the hash side explains it, but the `Dictionary` hit rows moved 30% between runs, so confirm on HX 370 before quoting.

**⏳❗ The ranking flips when the lookup feeds a dispatch (B550H / Zen 3, `TypeKeyDispatchBenchmark`):** the table above measures the lookup alone. A real registry **invokes** what it looked up (a factory delegate). Looking up 5 types in sequence from a `ConcurrentDictionary<K, Func<object>>` and invoking the result, the `RuntimeTypeHandle` key is 0.62x when one type repeats (4.33 vs 5.70 ns) but **2.24x slower when the 5 types alternate (15.64 vs 6.98 ns)**. Keys that avoid the identity hash have no such penalty: an `IntPtr` key over `TypeHandle.Value` is 0.69x (4.78 ns, but the table no longer references the `Type`), and a `Type` key with a `TypeHandle.Value` comparer is 1.00x. Hash acquisition alone and the lookup alone are fast on every path; the flip appears only for the combination of a value-type key hashed through the identity-hash FCall and a polymorphic call whose target follows the key (the handle variant has the shortest instruction stream). The real `ServiceRegistry.Activate` in BunnyTail.DependencyInjection reproduced it (current 9.79 ns → handle 18.39 ns, comparer 7.82 ns). → [Run 3](benchmarks/results/TYP-07-TypeKey.md)

**Which to use:** to speed up a `Dictionary` / `ConcurrentDictionary` without replacing it, key by `RuntimeTypeHandle` (callers pass `type.TypeHandle`) when the looked-up value is **used as a value**; in a registry that **dispatches on** the looked-up value, the flip above applies, so use a `TypeHandle.Value` comparer or an `IntPtr` key (if no collectible ALC). For more than that, the hand-written table. **The two decisions are independent.** Under NativeAOT the virtual-call gap vanishes as it did for TYP-07 itself, so the ranking may change — confirm in the final run. → [Results](benchmarks/results/TYP-07-TypeKey.md)
