...
📉 **実測・不採用の理由:** 速度・コードサイズとも完全に同値。JIT が `typeof(X)` 自体を定数(型ハンドル直値)に畳み込むため、手動キャッシュに省くべきコストが存在しない。
...
📉 **実測・不採用の理由:** AsSpan 化(約 2 倍)で改善は頭打ちになり、その先の ref ウォークは差なし。コードサイズだけが増える。
...
📉 **実測・不採用の理由:** 全形式が同一性能に収束する(サイズ 256 以上で完全一致)。JIT が同等のループ構造へ正規化するため、構文選択に最適化余地がない。

✅ **代わりにやること:** 可読性で選ぶ。効くのは構文でなくデータアクセス形(MEM-01 / COL-01 / MEM-04)。
...
✅ **代わりにやること:** Span / ref ベースで書く。再解釈は `MemoryMarshal.Cast`(ゼロコスト実測済み)、unmanaged 読み書きは SEQ-01 / SEQ-03。
...
📉 **実測・不採用の理由:** 呼び出しがインライン化される状況では readonly の有無による差は測定不能(全バリアント 0.006〜0.016ns)。
...
📉 **実測・不採用の理由:** 可変長では 0.98〜1.03 倍で同速(両者とも同じ Memmove に到達)。JIT が展開できる定数長 16B でも 0.83 倍・コードサイズ 45B 減の微差に留まり、安全性(境界チェックなし・型情報なし)を捨てる対価に見合わない。`Array.Copy` はコードサイズ肥大(1.7KB)で最遅。
