...
| R-14 revival check: constant-length copies (8 / 16 / 64 B) | 8 B 0.89x / 16 B 0.94x with overlapping CIs (0.81x in another run - not reproducible); 64 B **1.07x slower**, non-overlapping | Code size 52-64 B vs 96-102 B - **different code** | ❌ **No revival**. The sub-0.05 ns edge at small constants sits at the resolution limit and reverses by 64 B; code size alone does not outweigh the safety loss |
...
| ⑥ | MEM-04 size-dependence re-verification | in vs by-value now also measured at 16 / 128 / 256 bytes - is there any size where the copy shows up past the call overhead? | MEM-04 | ✅ Re-verified: in wins from 64 B (0.32-0.48x) and already at 16 B (0.84x); by-value is alignment-sensitive and unstable across runs while in stays flat (~1.2 ns) |
| ⑥ | R-14 revival check (constant-size copies) | CopyBlockUnaligned vs Span.CopyTo at constant 8 / 16 / 64 B - how far up does the real win extend, for generated code with guaranteed lengths? | LAB-CopyBlockUnaligned | ❌ No revival: the 8-16 B edge is <0.05 ns with overlapping CIs and does not reproduce; at 64 B CopyBlockUnaligned is 1.07x slower |
| ⑥ | R-08 revival check (larger Frozen tables) | FrozenDictionary build/lookup now also measured at 1024 string keys - does the lookup side start to win where construction can amortize? | COL-02 | ❌ No revival: at 1024 the lookup is 1.19x SLOWER (non-overlapping CIs) on top of a 5.3x build cost - the rejection is strengthened |
