...
| ⑥ | MEM-04 size-dependence re-verification | in vs by-value now also measured at 16 / 128 / 256 bytes - is there any size where the copy shows up past the call overhead? | MEM-04 | 🔬 Awaiting the next measurement run |
| ⑥ | R-14 revival check (constant-size copies) | CopyBlockUnaligned vs Span.CopyTo at constant 8 / 16 / 64 B - how far up does the real win extend, for generated code with guaranteed lengths? | LAB-CopyBlockUnaligned | 🔬 Awaiting the next measurement run |
| ⑥ | R-08 revival check (larger Frozen tables) | FrozenDictionary build/lookup now also measured at 1024 string keys - does the lookup side start to win where construction can amortize? | COL-02 | 🔬 Awaiting the next measurement run |
