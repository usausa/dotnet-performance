...
**対策:** Source Generator による静的アクセサ生成 / `Action<T, TValue>` 形式の事前デリゲート登録 / インターフェース契約への移行。対象が既知の非公開メンバーであれば `[UnsafeAccessor]`([README](../README.ja.md#️-typ-03-unsafeaccessor非公開メンバーへの直接アクセス) の TYP-03)がリフレクション不要かつ AOT 互換の代替になる。
