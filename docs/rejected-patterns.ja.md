...
📉 **実測・不採用の理由:** index を持ち回る素の `for` 333.3 ns に対して復元形は 507.7 ns(**1.52 倍**、信頼区間非重複)。コードサイズも 64 → 82 B(22 → 27 命令)と増える。x86-64-v3 では 1.45 倍でコードサイズは同一だったので、**新しいコアほど差が縮むのではなく開いている** — 復元は減算とシフトを**要素ごとに依存チェーン上へ**足すため、広いコアでも隠せない(削る作業が独立している STK-11 は同じ 2 機で逆に差が縮む)。R-02 と同じ理由で、索引形は JIT にとって最も扱いやすい形であり、ref 中心に書き換えると index が高くつく。
