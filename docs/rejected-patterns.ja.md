...
**ラッパーコレクション(`ReadOnlyCollection<T>`)は添字のまま:** .NET 10 の性能記事は「配列のインターフェース実装が devirt されるようになり、`int[]` を包んだ `ReadOnlyCollection<int>` では foreach が添字を上回る」としているが、**x86-64-v4 / x86-64-v3 のどちらでも再現しなかった**。foreach は添字の 1.21 倍遅く(558.6 vs 460.0 ns、x86-64-v3 では 1.26 倍)、列挙子 32 B の確保が残り、コードサイズは 664 B vs 134 B。生成コードを見ると devirt 自体はラッパーの `IList<T>` フィールド越しにも届いている(両形とも `int[]` のガード付き devirt、foreach では `SZGenericArrayEnumerator<int>` の MoveNext/Current がインライン化)が、**列挙子オブジェクトのスタック化は起きず** `CORINFO_HELP_NEWSFAST` が残るため、ヒープ上の `_index` の再ロードとチェック 3 本を毎要素払う。添字形はガード 2 本 + 境界チェック 1 本で確保なし。ラッパー越しの反復は引き続き添字で書く。→ [LAB-ReadOnlyCollectionLoop.md](../benchmarks/results/LAB-ReadOnlyCollectionLoop.md)
...
📌 **「ポインタの方が速い」という外部報告の検証:** NDepend の記事(2026)は little-endian Int32 の一括デコードで「Span 0.26 倍 / ポインタ 0.19 倍」を報告し、ポインタ優位としている。同じ 1024 要素で 4 形を並べて測った。

| 形 | 時間 | 比率 | コードサイズ |
|---|---:|---:|---:|
| 1 バイトずつシフト合成(基準) | 718.2 ns | 1.00 | 200 B |
| 要素ごとに `BinaryPrimitives.ReadInt32LittleEndian` | 364.0 ns | 0.51 | 85 B |
| **`MemoryMarshal.Cast<byte, int>` + 索引ループ** | **215.2 ns** | **0.30** | **54 B** |
| `fixed` + `int*` | 249.0 ns | 0.35 | 97 B |

**`Cast` がポインタより速い(ポインタは 1.16 倍遅い、信頼区間非重複。x86-64-v3 では 2.0 倍)。** 記事の「Span 0.26 倍」は `Cast` 形(0.26〜0.30 倍)に相当し、「ポインタ 0.19 倍」は 2 機とも再現しなかった(0.35 / 0.52 倍)。生成コードでは `Cast` 形の内側ループが 5 命令(バイトオフセットで進めて残数を `dec`)なのに対し、ポインタ形は `int` 添字の `movsxd` が毎要素の依存チェーンに乗る 6 命令で、`fixed` のスタックフレームも付く。**一度 `Cast` してから索引で回せばポインタに勝つ**という本項の結論そのもの。→ [LAB-Int32Parse.md](../benchmarks/results/LAB-Int32Parse.md)

📌 **C# 16 の unsafe 再定義について:** 同記事によれば C# 16 では `unsafe` がメンバ単位になり、ポインタ**型**そのものは unsafe でなくなって逆参照だけが unsafe 文脈を要求する方向(`/// <safety>` による契約の明文化、unsafe 文脈外からの呼び出しはエラー)。本項の「unsafe コンテキストの導入コスト(監査・安全性)」という論拠はそれに伴って弱まるが、**不採用の根拠は実測(同速か遅い)であり、言語仕様の変更で覆るものではない**。C# 16 が確定した時点でこの段落の表現を見直す。
...
📌 **.NET 10 で境界チェックが消える範囲・消えない範囲(生成コードで確認):** 「消えるパターン集」として整理された外部記事(Zenn、2026-04)と .NET 10 の JIT 変更のうち、実装判断に効く 2 点を確認した。

| 形 | string / 配列 | `ReadOnlySpan<char>` |
|---|---|---|
| `prefix.Length < path.Length ? path[prefix.Length] : -1`(**別のシーケンスの Length を添字に使う**) | **消える**(22 B、RNGCHKFAIL なし) | **残る**(48 B、`cmp/jae` + `CORINFO_HELP_RNGCHKFAIL` + スタックフレーム) |

**同じガードを書いても Span だけ境界チェックが残る。** ガードと境界チェックは同じ 2 レジスタの比較なのに JIT は Span では融合しない。時間への影響は x86-64-v4 で 1.09 倍(x86-64-v3 では 2.05 倍)とコア依存だが、**命令列の差は両機で同一**。ホットパスで「他のシーケンスの長さを添字に使う」なら、Span より string / 配列で受けるか、添字を自身の `Length` から導く形に直す。

`switch (span.Length) { 4 => span[0] + span[1] + span[2] + span[3], _ => -1 }` は .NET 10 から `if (span.Length == 4)` ガードと同じく**チェックなし**(32 B vs 31 B で命令列も同等)。長さで分岐する整形・解析コードは switch で書いてよい。

なお同記事の「`(uint)` キャスト」「末尾を先に触る」は、生成コード上は消えても**時間には出ない**ことを本項と R-18 で実測済み。asm で消えることと速くなることは別に扱う。→ [LAB-BoundsCheckPattern.md](../benchmarks/results/LAB-BoundsCheckPattern.md)
...
### R-22: 汎用キーの自作ハッシュ表(ankerl::unordered_dense 方式)

🎯 **狙い:** `Dictionary<TKey, TValue>` を、Robin Hood 探索 + 8 ビット fingerprint を持つ密なメタデータ配列と、キー/値を別配列に詰めるレイアウト(ankerl::unordered_dense)の自作表で置き換えて探索を速くする。外部記事(2024)は 1024 件で探索 4.88 μs vs `Dictionary` 18.61 μs(約 3.8 倍)と報告している。

📉 **実測・不採用の理由:** 同じ 1024 個の string キー・同じハッシュ関数で並べると**逆に遅い**。出典の「3.8 倍高速」と正反対のため x86-64-v4 / x86-64-v3 の 2 機で測ったが、符号は両機で一致し、新しいコアほど差が開く。

| 操作 | `Dictionary<string, int>` | Ankerl 方式 | 比率 |
|---|---:|---:|---:|
| 探索(全件ヒット) | 3.92〜3.93 μs | 5.82 μs | **1.48 倍 遅い**(x86-64-v3: 1.26〜1.66 倍) |
| 探索(全件ミス) | 3.38 μs | 5.33 μs | 1.58 倍 遅い(x86-64-v3: 1.14〜1.30 倍) |
| 構築 | 6.59 μs | 7.98 μs | 1.21 倍 遅い(x86-64-v3: 同等) |

差はハッシュ関数ではない。`Dictionary` は string キーで非ランダム化ハッシュを使い、Ankerl 側は `string.GetHashCode`(ランダム化)しか使えないため、`StringComparer.Ordinal` を渡して `Dictionary` も同じランダム化ハッシュにした行を足したが、それでも Ankerl は **1.48 倍遅い**(5.82 vs 3.92 μs、信頼区間非重複。x86-64-v3 でも 1.40 倍)。**表のレイアウト自体が .NET 10 の `Dictionary` に勝っていない。**

記事の 3.8 倍は `Dictionary` 側の基準値(1024 探索で 18.6 μs)が原因で、この環境の `Dictionary` は同じ仕事を 3.9 μs(x86-64-v3 でも 5.7〜7.8 μs)でこなす。**比較対象の基準値が外れ値だった**。

✅ **代わりにやること:** 汎用キーは `Dictionary<TKey, TValue>` のまま。既知キー集合の名前解決は COL-04(サンプリングハッシュ、`Dictionary` 比 0.60〜0.62 倍)、`Type` キーは TYP-01。自作するなら「特定のキー分布で `Dictionary` に実測で勝った」場合に限り、汎用のレイアウト差では勝てない。

🔗 **測定記録:** [LAB-HashTableDesign.md](../benchmarks/results/LAB-HashTableDesign.md)

---

### R-23: InlineArray + ヒープスピルの小容量リスト(SmallVec)

🎯 **狙い:** 「先頭 N 要素は struct 内の `[InlineArray]` に持ち、超えたら配列へ移す」小ベクタ形(SmallVec)で、要素数が少ないうちは `List<T>` のヒープ確保を省き、超過時もそのまま動くようにする。外部記事(Qiita)は収まる範囲で `List<T>` の 166% 高速と報告している。

📉 **実測・不採用の理由:** 収まるうちは出典どおり勝つが、**溢れた瞬間に容量指定の `List<T>` に負ける**。この非対称は x86-64-v4 / x86-64-v3 の 2 機で符号が一致し、確保バイト数も同一。

| 要素数 | `List<int>` | `List<int>(capacity)` | InlineList(8 要素インライン) |
|---:|---:|---:|---:|
| 4(収まる) | 8.8 ns / 72 B | 8.1 ns / 72 B | **4.3 ns / 0 B(0.49 倍)** |
| 32(溢れる) | 58.5 ns / 368 B | **25.0 ns / 184 B(0.43 倍)** | 44.1 ns / 240 B(**容量指定に 1.76 倍負け**、x86-64-v3: 1.62 倍) |

溢れると、インライン分のコピー + 再確保 2 回が正しい容量での 1 回確保より高くつき、確保も 240 vs 184 B で多い。勝てるのは「上限が既知で、インライン容量をそれに合わせられる」場合だけだが、**上限が既知ならスピル経路そのものが不要で、STK-08 の固定長 InlineArray で足りる**。上限が読めない用途では `List<T>(capacity)` / BUF-05 に負ける。つまり STK-08 と BUF-05 の間に出番が残らない。加えて値渡しでコピーされるとインライン形と配列形で書き換えの見え方が変わるため、`ref struct` にして複製を型で禁止する必要がある(出典自身の注意点)。

✅ **代わりにやること:** 上限既知なら STK-08(`[InlineArray]` の固定長バッファ)、上限不明なら `List<T>(capacity)` か BUF-05 の段階戦略(stackalloc / ArrayPool)。

🔗 **測定記録:** [LAB-InlineList.md](../benchmarks/results/LAB-InlineList.md)
