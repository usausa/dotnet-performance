...
判断基準の「信頼区間が重なり、生成コードも一致すれば**差なし**」は正しいが、**逆向きの推論 —「生成コードが一致するのだから性能も同じはず」— は成立しない**。VEC-02 の検証がその反例で、`Vector128.Shuffle` と `Ssse3.Shuffle` は**逆アセンブリが完全一致**(190 B、同じ `vpshufb`)にもかかわらず 121.53 vs 64.35 ns と **1.76 倍**の再現性ある差が出た。原因はホットループの配置で、片方は 64 バイトの命令フェッチ窓に収まり、もう片方は境界を跨いでいた。
...
| 7-1 `scoped` の有無(span / ref 引数) | 69.22 vs 70.00 ns / 2.276 vs 2.296 ns、信頼区間重複 | caller / callee とも**命令列完全一致**(89 / 35 B、50 / 38 B) | ❌ **差なし**(純粋なコンパイル時契約。安全性ツールとして STK-01 に収録) |
| 7-1 `[UnscopedRef]` ref 返しアクセサ | 0.5013 vs 0.5349 ns、信頼区間重複 | **別物**だが命令数は同じ 7、コードは 85 → 88 B と増加 | ❌ **不採用**(どの軸にも改善なし → R-20) |
| 7-7 `Unsafe.BitCast` vs `Unsafe.As` | 241.5 vs 243.9 ns、信頼区間重複 | **命令列完全一致**(具象 57 B ×3、ジェネリック 21 B ×2) | ❌ **差なし** → ただし**安全性軸で採用**(同一コードは「乗り換えコストゼロ」の証明) |
| 7-3 `GetValueRefOrNullRef` の読み取り経路 | 0.98〜1.00 倍、信頼区間重複 | 命令数一致(199 vs 199)、コードは +16 / −14 / −4 B と増減混在 | ❌ **差なし**(更新経路のみ採用 → COL-07) |
| 7-9 `Vector128.Shuffle` vs `Ssse3.Shuffle` | 121.53 vs 64.35 ns、**信頼区間非重複** | **命令列完全一致**(190 B) | ⚠️ **配置由来**。API 差ではない(落とし穴 10 を参照) |
...
| ⑦ | ref フィールドの構造読み | R-12 が名指しした「フィールド粒度の読み」で索引形に勝つか | STK-01 / R-12 | ✅ 収録([STK-11](../README.ja.md)、0.75 倍) |
| ⑦ | `GetValueRefOrNullRef` + `IsNullRef` | 存在確認つき更新を探索 1 回に畳めるか | COL-01 | ✅ 収録([COL-07](../README.ja.md)、更新 0.48〜0.62 倍。読み取りは差なし) |
| ⑦ | struct レイアウト(サイズ・フィールド順) | 32 → 24 バイトが走査・引数渡しに出るか | MEM-02 / MEM-04 | ✅ 条件付き収録([MEM-05](../README.ja.md)、散在 0.67〜0.71 倍。逐次は誤差) |
| ⑦ | false sharing / キャッシュラインパディング | 罰則の大きさ。64 バイトで足りるか | CON-01 | ✅ 収録([CON-03](../README.ja.md)、最大 29.7 倍。128 バイト必須) |
| ⑦ | `Memory<T>` の `.Span` コストと配列橋渡し | ホイストの効果、裏の実体依存、`TryGetArray` | BUF-04 | ✅ 収録([BUF-08](../README.ja.md)、要素ごとで 2.96 倍) |
...
| ⑦ | 固定幅組み込み関数(Shuffle) | `Vector<T>` で書けないレーン置換の効果 | VEC-01 | ✅ 収録([VEC-02](../README.ja.md)、0.46 倍。ISA 直呼びの優位は配置由来で否定) |
...
| ⑦ | `Unsafe.Unbox<T>` | 既存ボックスを再確保せず更新できるか | STK-05 | ✅ 収録(STK-05 拡張、0.18 倍・割り当てゼロ) |
