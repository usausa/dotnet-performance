...
|---|---:|---:|---:|---:|
| 先頭付近(index 4) | 1.824 ns | **1.328 ns** | **0.73** | 566 → **399 B** |
| 末尾付近(index 248) | 7.018 ns | 7.563 ns | 1.08(誤差内) | 547 → **382 B** |
| 不一致(全走査) | 6.438 ns | 5.826 ns | 0.90(誤差内) | 551 → **394 B** |

**早期に一致するデータほど有利**(先頭一致は信頼区間非重複で 0.73 倍)で、悪化するケースは無い。**コードサイズは全ケースで約 30% 小さい**ため、時間差が誤差に収まる用途でも `ContainsAny` を既定にしてよい。→ [測定結果](benchmarks/results/TXT-08-ContainsAny.md)
