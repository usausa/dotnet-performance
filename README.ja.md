...
**.NET 10 での前提の変化(⏳ 仮測定: B550H / x86-64-v3、HX 370 で差し替え):** 動的 PGO の条件付きエスケープ解析により、**`IEnumerable<int>` 型のフィールドが常に `int[]` を指している(単相)なら、`foreach` の列挙子はスタック化されて確保が消える** — 324 ns / **0 B**(配列を直接回す 311 ns と同等)。つまり「`IEnumerable<T>` の foreach はヒープ確保」という前提は、単相の呼び出し点では成り立たなくなった。

| 形 | 時間 | 確保 | コードサイズ |
|---|---:|---:|---:|
| `int[]` を直接 foreach | 311 ns | 0 B | 32 B |
| `IEnumerable<int>` フィールド(常に `int[]`) | 324 ns | **0 B** | 206 B |
| `IEnumerable<int>` フィールド(`int[]` と `List<int>` が交互) | **3,282 ns** | **36 B** | 660 B |
| struct enumerator(本パターン) | 417 ns | 0 B | 55 B |

ただし**多相になった瞬間に 10 倍・確保ありへ戻る**。STK-03 の価値は「PGO の観測に依存せず、多相でも常に確保ゼロ・直接呼び出しを保証する」点へ移った。自分の型の API 設計としては引き続き struct enumerator を既定にし、**他所の `IEnumerable<T>` を消費する側で列挙子確保を気にする必要は単相なら無くなった**、と読む。→ [測定結果](benchmarks/results/LAB-EnumerableEscape.md)
...
**派生: InlineArray + ヒープへのスピルによる小容量リスト(⏳❗ 想定外・本検証待ち: 「収まれば勝つ」は出典どおりだが、「溢れると容量指定 List に負ける」は出典に無い結果で採用条件を左右する。HX 370 で確認後に「条件付き採用」を確定。数値は B550H / x86-64-v3 の仮値):** 「先頭 N 要素は struct 内の `[InlineArray]` に持ち、超えたら配列へ移す」という小ベクタ形(SmallVec)を `List<T>` と比べた。

| 要素数 | `List<int>` | `List<int>(capacity)` | InlineList(8 要素インライン) |
|---:|---:|---:|---:|
| 4(収まる) | 21.6 ns / 72 B | 16.5 ns / 72 B | **7.6 ns / 0 B(0.35 倍)** |
| 32(溢れる) | 133.5 ns / 368 B | **55.6 ns / 184 B(0.42 倍)** | 90.2 ns / 240 B(0.68 倍) |

**収まるうちは圧勝、溢れた瞬間に容量指定の `List<T>` に負ける**(インライン分のコピー + 再確保 2 回が、正しい容量での 1 回確保より高い)。採用条件は「上限が既知で、インライン容量をそれに合わせられる」場合に限る。上限が読めないなら `List<T>(capacity)` か BUF-05。なお値渡しでコピーされるとインライン形と配列形で書き換えの見え方が変わるため、**`ref struct` にして複製を型で禁止する**。→ [測定結果](benchmarks/results/LAB-InlineList.md)
...
**.NET 10 のエスケープ解析でも指針は変わらない(⏳❗ 想定外・本検証待ち: .NET 10 の公式記事は「消去される」としており、この環境の結果と食い違う。HX 370 で再現しなければ本文を「.NET 10 でも消去は起きない」で確定、再現すれば書き換える。数値は B550H / x86-64-v3 の仮値):** .NET 10 は「エスケープしないデリゲートをスタック化する」と発表されているが、同一メソッド内で生成して即呼び出す形で測っても**この環境では消去は起きなかった**。ループ内変数をキャプチャするラムダは反復ごとに **88 B(表示クラス 24 B + デリゲート 64 B)** をそのまま確保する(64 反復で 5,632 B)。メソッド内変数のキャプチャは Roslyn が表示クラスにデリゲートをキャッシュするため 96 B/呼び出しで頭打ち。**static + TState だけが 0 B**。

| 形 | 確保 / 呼び出し | 備考 |
|---|---:|---|
| ループ内変数をキャプチャ | **5,632 B**(64 反復 × 88 B) | .NET 10 の消去は再現せず |
...
| R-22 | 汎用キーの自作ハッシュ表(ankerl::unordered_dense 方式) | ⏳❗ 仮判定(不採用見込み)。出典の 3.8 倍高速と逆に、同一ハッシュでも `Dictionary` より 1.40 倍遅い。本検証で確定 |
