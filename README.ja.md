...
| `AggressiveOptimization` | Dynamic PGO を無効化するため**かえって遅くなりうる** | **実測: 予測どおりで、JIT 側の代償は深刻。** PGO に依存する形(ループ内のインターフェースディスパッチ)では JIT で **5.79 倍**、依存しない直線的な算術でも 1.26 倍。AOT では 0.95 / 1.02 倍で完全に不活性 | JIT では原則使わない。AOT では本当に何も起きない。→ [測定結果](benchmarks/results/LAB-AotAssumptions.md) |
